...
#
# NOTE:
# Depending on the nature of your sample design, and if any duplicates are discovered in the script"03_check_duplicates.R",
# you may need to run through the following script multiple times. It's very important that you explicitely state which stage you are in
# prior to saving and loading files. 
#
# Note: more filtering steps can be added as needed. 
...
project.merged.nodups <- "dcor_wpac_final"
...
label.path.final <- "data-raw/metadata/dcor.wpac.labels_zpad_merged_nodups.txt"
...
#Uncomment and run this when all duplicates have been identified and taken care of
haplo.read.tbl <- prepHaplotFiles(
run.label = project.merged.nodups,
sam.path = sam.path,
out.path = results.r.path,
label.path = label.path.final,
vcf.path = vcf.path,
app.path = app.path,
n.jobs = parallel::detectCores() # Use all available cores
)
...
tgt.initial.merged.nodups <- mplot2tgt(project = project.merged.nodups, out.path=results.r.path, AB.min.het = AB.min.het, AB.max.homo = AB.max.homo, min.read.depth = min.read.depth)
...
tgt.initial.merged.nodups <- tgt.initial.merged.nodups %>%
  group_by(locus) %>%
  filter(!all(is.na(gt))) %>%
  ungroup()
...
write_rds(tgt.initial.merged.nodups, file=paste0("results-R/tgt.initial.", project.merged.nodups, ".rds")) 
...
num.locs.initial.merged.nodups<- tgt.initial.merged.nodups %>% filter(!is.na(gt)) %>% distinct(locus) %>% summarise(x = n()) %>% pull(x)
...
run_qc_analysis(tgt.initial.merged.nodups, "initial", num.locs.initial.merged.nodups, project=project.merged.nodups)
...
tgt.pass1.loci.merged.nodups <- filter(tgt.initial.merged.nodups, locus %in% loci.to.keep.pass1) #use this when samples have beem merged and duplicates have been taken care of
...
#  filter(!Indiv %in% pass1.manual.inds.to.remove) %>% #Only include this line if there are individuals to remove manually
  pull(Indiv)
#tgt.pass1.merged <- filter(tgt.pass1.loci.merged.nodups, Indiv %in% inds.to.keep.pass1)
tgt.pass1.merged.nodups <- filter(tgt.pass1.loci.merged.nodups, Indiv %in% inds.to.keep.pass1)
...
message(paste("  - First pass removed", length(unique(tgt.initial.merged.nodups$locus)) - length(loci.to.keep.pass1), "loci and",
              length(unique(tgt.initial.merged.nodups$Indiv)) - length(inds.to.keep.pass1), "individuals."))
...
write_rds(tgt.pass1.merged.nodups, file=paste0("results-R/tgt.pass1.", project, ".rds"))

# Step 2C: Re-run QC analysis on the intermediate 'pass1' dataset
#num.locs.pass1.merged <- length(unique(tgt.pass1.merged$locus))
num.locs.pass1.merged.nodups <- length(unique(tgt.pass1.merged$locus))

#run_qc_analysis(tgt.pass1.merged, "pass1", num.locs.pass1.merged, project=project)
run_qc_analysis(tgt.pass1.merged.nodups, "pass1", num.locs.pass1.merged.nodups, project=project)


#✅ PART 2 COMPLETE.
#--------------------------------------------------------------------------
#🛑 ACTION REQUIRED BEFORE PART 3: 
#1. Examine all questionable haplotypes present in the 'genos.to.check' file after the first pass filtering
#2. Take note of any loci that are problematic to remove later on since they may not get removed with the first pass filter. 
...
final.manual.loci.to.remove <- c("locus016") # <-- Add any other loci to remove
...
genos_to_change <- read.csv('data-raw/genos_to_change.csv') #may give a warning but should still import the table
#for (i in 1:nrow(genos_to_change)){
  idx <- which(tgt.pass1.merged$locus == genos_to_change$locus[i] & tgt.pass1.merged$Indiv == genos_to_change$Indiv[i])
  tgt.pass1.merged$gt[idx] <- genos_to_change$gt[i]
  message("Changed genotypes:")
  print(tgt.pass1.merged[idx, ])
}
for (i in 1:nrow(genos_to_change)){
  idx <- which(tgt.pass1.merged.nodups$locus == genos_to_change$locus[i] & tgt.pass1.merged.nodups$Indiv == genos_to_change$Indiv[i])
  tgt.pass1.merged.nodups$gt[idx] <- genos_to_change$gt[i]
  message("Changed genotypes:")
  print(tgt.pass1.merged.nodups[idx, ])
}
...
loci.to.keep.final <- loc.sum.pass1 %>%
  ...
  pull(locus)
#tgt.final.loci <- filter(tgt.pass1.merged, locus %in% loci.to.keep.final)
tgt.final.loci <- filter(tgt.pass1.merged.nodups, locus %in% loci.to.keep.final)
...
run_qc_analysis(tgt.final, "final", num.locs.final, project=project)
