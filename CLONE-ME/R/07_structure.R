...
# Save the Evanno metrics table. Update the path accordingly (noloc, wloc, popflag, etc)
evanno_path <- file.path(results_raw_path, paste0(run_label, "_evanno_metrics_sr_no_loc.csv"))
...
delta_k_plot <- ggplot(evno$df, aes(x = k, y = delta.k)) +
  ...
  theme_minimal()
delta_k_plot
#Update the path accordingly (noloc, wloc, popflag, etc)
plot_path <- file.path(results_raw_path, paste0(run_label, "_delta_k_plot_sr_noloc.pdf"))
...
q_matrix_5<-sr$dcor.wpac.full.merged.noloc.structureRun.k5.r1
head(q_matrix_5)
colnames(q_matrix_5)<-c("Indiv", "%Missing", "Stratum", "K=1", "K=2", "K=3", "K=4", "K=5")
