...
inputDir <- args[1]
# the directory to save output foder that contains all the bar plots for different locus
SaveDir <- args[2]
# check if the command-line arguments are correct
if (length(args)!=2){
  stop("The command-line arguments are not correct!\n
       The first argument should be the directory of the genotype output generated by MEGASAT\n
       The second argument should be the diretory to save your plots output\n")
}
##get the directory of length distribution subfolder
genotypeDir = paste(inputDir,"/length_distribution",sep="")
# get the files whose names are in a  pattern from the input folder
InfileList = list.files(genotypeDir,pattern='Genotype_.*\\.txt',recursive=TRUE)
# get the number of files in the input folder
numFile=length(InfileList)
if(numFile==0){
  stop("The directory of the genotype output is wrong!")
}
...
if(length(file_name)==0){
  stop("The genotype output does not contain Ratios_Threshold_XX.txt")
}
...
FolderName <- str_extract(inputDir, "([^/]+$)")
# check if the the saving directory is correct
if(!dir.exists(SaveDir)){
  stop("The direcoty to save your plots output does not exist")
}
...
if(file.copy(from=paste (inputDir, "Genotype.txt", sep = "/"), to=paste (plotDir, "Genotype.txt", sep = "/"),overwrite =TRUE,copy.mode = TRUE)==FALSE){
  print("Cannot copy Genotype.txt to plot directory since the genotype output folder does not contain Genotype.txt!")
}
...
MyData <- read.csv(paste(genotypeDir,"/",InfileList[1],sep=""), header=TRUE, sep=",")
...
for(j in 1:length(lociName)){
  ...
  for(i in InfileList){
    ...
    s1 = unlist(strsplit(s1,split="Genotype_",fixed=TRUE))[2]
    titleName[[k]]<- s1
    # read each csv file to get corresponding data for each locus
    db<-read.csv.sql(paste(genotypeDir,"/",i,sep=""), sql = sprintf("select * from file where Microsatellite = '%s'", lociName[[j]]), header = TRUE, sep = ",",drv = "SQLite")
    if(ncol(db)==3){
      db$X0<-0
    ...
    num_var = nrow(SISLData)
    ## if the number of variable is too large, we need to decrease the letter size
    if(num_var>=30){
      q <- ggplot(data=SISLData, aes(x=variable, y=value, fill=bar_color)) +
        ...
        theme(text = element_text(size=360/num_var),
              axis.text.x = element_text(angle = 90, hjust = 1))
    } else{
      q <- ggplot(data=SISLData, aes(x=variable, y=value, fill=bar_color)) +
        scale_fill_manual(values = colorScheme,breaks= c("A","B","C","D")) +
        geom_bar(stat="identity", show.legend=FALSE)+
        xlab(bquote(atop("length", ~ .(score[[1]][1]) ~ ',' ~ .(score[[1]][2]))))+
        ylab("depth")+
        ggtitle(paste(titleName[[k]],"\n",db[,ncol(db)-1])) +
